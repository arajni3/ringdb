...
consteval uint64_t compile_time_num_bits(uint64_t num_elems, float false_pos_prob) {
    return gcem::ceil((static_cast<int64_t>(-num_elems) * gcem::log(false_pos_prob))
    / LOG_TWO_SQUARED);
}

consteval unsigned int compile_time_num_hashes(uint64_t num_elems, float false_pos_prob) {
    uint64_t m = compile_time_num_bits(num_elems, false_pos_prob);
    unsigned int k = gcem::round(LOG_TWO * m / num_elems);
    return k;
}

consteval unsigned long compile_time_bloom_length(uint64_t num_elems, float false_pos_prob) {
    uint64_t m = compile_time_num_bits(num_elems, false_pos_prob);
...
struct BloomFilter {
    ...
    float false_positive_probability = false_pos_prob;
...
template<uint64_t num_elements, float false_pos_prob>
float bloom_filter_current_false_positive_rate(BloomFilter<num_elements, false_pos_prob> *bf);
...
template<uint64_t num_elements, float false_pos_prob>
float bloom_filter_jaccard_index(BloomFilter<num_elements, false_pos_prob> *bf1, BloomFilter<num_elements, false_pos_prob> *bf2);
...
float bloom_filter_current_false_positive_rate(BloomFilter<num_elements, false_pos_prob> *bf) {
    ...
    float d = -num / (float) bf->number_bits;
    float e = exp(d);
...
uint64_t bloom_filter_estimate_elements_by_values(uint64_t m, uint64_t X, int k) {
    ...
    float log_n = log(1 - ((float) X / (float) m));
    return (uint64_t)-(((float) m / k) * log_n);
}
